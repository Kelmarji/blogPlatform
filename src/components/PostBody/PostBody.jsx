...
const PostBody = ({ slug }) => {
  ...
  const validate = (txt, func) => (txt ? func(txt) : 'untiled/undescrtipted');
  ...
  return (
    <li key={slug} id={slug} className={s.listItem}>
        ...
        <div className={s.HeaderTitle}>
          <div className={s.HeaderLeft}>
            ...
            <div
              ...
              onClick={() => {
                BlogApi.unlikeLikePost(liked, token, slug).then((articl) => setLiked(articl.favorited));
                changeLike(!liked);
              }}
            >
              <label
                onClick={() => changeLike(!liked)}
                className={favorited ? s.LikeCounterCheked : s.LikeCounterUncheked}
                htmlFor={id}
              >
                <span>{favoritesCount > 0 ? favoritesCount : 0}</span>
              </label>
            </div>
          </div>
          <ul className={s.tags}>{renderTags(tagList.slice(0, 6))}</ul>
        </div>
      ...
      <div className={s.DescPost}>
        <span className={s.DescPost}>{validate(description, descr)}</span>
      </div>
    </li>
  );
};
